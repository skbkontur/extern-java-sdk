...
* вызвать статический метод *usingCertificate* класса *CertificateAuthenticationProvider* и передать ему следующие параметры, который:
  * *certificateProvider*: *CertificateProvider* -  объект, реализующий метод `QueryContext<byte[]> getCertificate(String thumbprint)`. Метод должен принимать параметр отпечаток сертификата, а возвращать контекст с сертификатом в DER-кодировке или ошибку, в зависимости от возвращаемого результата метода isFail;
...
* в полученный билдер с помощью соответствующих методов необходимо установить следующие провайдеры:
  * *ApiKeyProvider* - провайдер для получения идентификатора сервиса. [Описание см. выше](#apiKey);
  * *CryptoProvider* - реализацию криптографического провайдера. [Описание см. выше](#cryptoProvider);
  ...
  * *SignatureKeyProvider* - провайдер, предоставляющий отпечаток сертификата личного ключа. В простейшем случае можно использовать лямбда-выражение: ()->"XXXXXXXXXXXXXXXXXXXX".
