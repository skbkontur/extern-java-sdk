...
- `QueryContext<Company> create(QueryContext<?> cxt)`  
    Должен принимать параметр **QueryContext**, содержащий структуру данных **CompanyGeneral**, установленую с помощью метода **setCompanyGeneral**.  
...
Параметры:  
  ...
  - ИНН;  
  - КПП;  
  - сертификат подписи отправителя в кодеровке BASE64 без тегов;  
